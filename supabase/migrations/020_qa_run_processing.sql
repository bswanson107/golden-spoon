-- Golden Spoon — QA week processing as a SQL RPC
--
-- Runs the time-dependent side effects (kickoff win-% lock + auto-MNF / missed
-- assignment + scoring) against the simulated clock, entirely in the database.
-- This mirrors the TypeScript sync side effects (lockKickoffWinPcts +
-- markMissedPicks) but does not require the edge function to be deployed and
-- never touches the nflverse feed, so QA is self-contained and reliable.

create or replace function public.qa_run_processing(p_season_year integer)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  select count(*) into v_locks from upd;
  ...
  loop
    ...
    loop
      if exists (
        ...
        values (
          v_member.league_id, v_member.user_id, p_season_year, v_week.week,
